            ...
            <table class="table datatable table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $action->action_price }}</td>
                        <td>
                            <a href="{{ route('action.edit', $action->id )}}" data-bs-toggle="tooltip"
                                data-bs-placement="top" title="Edit" class="badge bg-warning border-0"><i
                                    class="bi bi-pencil-square me-1"></i></a>
                            <form id="deleted-form" action="{{ route('action.destroy', $action->id) }}" method="POST"
                                class="d-inline">
                                <button class="badge bg-danger  border-0" data-bs-toggle="tooltip"
                                    data-bs-placement="top" title="Hapus"
                                    onClick="return confirm(`Apakah Yakin hapus {{ $action->action_name}}?`)">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="number" class="form-control @error('action_price') is-invalid @enderror"
                                id="action_price" name="action_price">
                            @error('action_price')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
